...
#import "LDXMoveTableView.h"

static NSTimeInterval kLDXMoveCellAnimationTime = 0.25;

@interface LDXMoveTableView ()
...
@implementation LDXMoveTableView
...
- (void)ldx_addGesture
{
    _gesture = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(ldx_processGesture:)];
    _gesture.minimumPressDuration = _gestureMinimumPressDuration;
    [self addGestureRecognizer:_gesture];
}

- (void)ldx_processGesture:(UILongPressGestureRecognizer *)gesture
{
    switch (gesture.state) {
        case UIGestureRecognizerStateBegan:
        {
            [self ldx_gestureBegan:gesture];
        }
            break;
        case UIGestureRecognizerStateChanged:
        {
            if (!_canEdgeScroll) {
                [self ldx_gestureChanged:gesture];
            }
        }
            break;
        ...
        {
            [self ldx_gestureEndedOrCancelled:gesture];
        }
            break;
        default:
            break;
...
- (void)ldx_gestureBegan:(UILongPressGestureRecognizer *)gesture
{
    ...
    _tempView = [self ldx_snapshotViewWithInputView:cell];
    ...
    //隐藏cell
    cell.hidden = YES;
    [UIView animateWithDuration:kLDXMoveCellAnimationTime animations:^{
        _tempView.center = CGPointMake(_tempView.center.x, point.y);
    }];
}

- (void)ldx_gestureChanged:(UILongPressGestureRecognizer *)gesture
{
    CGPoint point = [gesture locationInView:gesture.view];
    ...
    if (currentIndexPath && ![_selectedIndexPath isEqual:currentIndexPath]) {
        //交换数据源和cell
        [self ldx_updateDataSourceAndCellFromIndexPath:_selectedIndexPath toIndexPath:currentIndexPath];
...
- (void)ldx_gestureEndedOrCancelled:(UILongPressGestureRecognizer *)gesture
{
    if (_canEdgeScroll) {
        [self ldx_stopEdgeScroll];
    }
    //返回交换后的数据源
...
- (UIView *)ldx_snapshotViewWithInputView:(UIView *)inputView
{
...
- (void)ldx_updateDataSourceAndCellFromIndexPath:(NSIndexPath *)fromIndexPath toIndexPath:(NSIndexPath *)toIndexPath
{
...
- (void)ldx_startEdgeScroll
{
    _edgeScrollTimer = [CADisplayLink displayLinkWithTarget:self selector:@selector(ldx_processEdgeScroll)];
    [_edgeScrollTimer addToRunLoop:[NSRunLoop mainRunLoop] forMode:NSRunLoopCommonModes];
}

- (void)ldx_processEdgeScroll
{
    [self ldx_gestureChanged:_gesture];
